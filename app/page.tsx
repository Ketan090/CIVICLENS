...
export default function Page(){
  ...
  useEffect(()=>{
    const poll=async()=>{ try{ const r=await fetch("/api/analyze",{cache:"no-store"}); const j=await r.json(); setStatus({reachable:!!j.unorouter?.reachable, model:j.unorouter?.model||null}); }catch{} };
  ...
  const analyze=async()=>{
    ...
    try{
      ...
      if(j.demo){ setRaw(j.message||"Uno Router not reachable"); setLoading(false); return; }
