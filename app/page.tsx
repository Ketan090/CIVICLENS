  ...
  return (<div className="min-h-screen bg-[#050608] text-white">
      ...
      <div ref={uploadSplit.ref} className="rounded-[28px] border border-white/10 bg-white/[.04] backdrop-blur overflow-hidden flex flex-col lg:flex-row">
        <div style={{flex:`0 0 ${uploadSplit.pct}%`}} className="p-6 min-w-0">
          <div onClick={(e)=>{ if((e.target as HTMLElement).closest('button')) return; fileRef.current?.click(); }} onDragEnter={(e)=>{e.preventDefault(); setDragOver(true);}} onDragLeave={(e)=>{e.preventDefault(); setDragOver(false);}} onDragOver={(e)=>{e.preventDefault(); e.stopPropagation(); setDragOver(true);}} onDrop={async(e)=>{e.preventDefault(); e.stopPropagation(); setDragOver(false); let f:any=e.dataTransfer.files?.[0]; if(!f && e.dataTransfer.items){ for(const it of Array.from(e.dataTransfer.items) as any){ if(it.kind==="file"){ const ff=it.getAsFile(); if(ff){ f=ff; break; } } } } if(f){ if(f.type && !f.type.startsWith("image/")){ alert("Please drop an image file (JPG/PNG/WEBP)"); return; } onFile(f); return; } const url=e.dataTransfer.getData("text/uri-list")||e.dataTransfer.getData("text/plain"); if(url && /^https?:\/\//i.test(url.trim())){ try{ const r=await fetch(`/api/proxy?url=${encodeURIComponent(url.trim())}`); if(r.ok){ const b=await r.blob(); const nf=new File([b],"drop.jpg",{type:b.type||"image/jpeg"}); onFile(nf); return; } }catch{} } }} className={`group relative rounded-[24px] border p-7 cursor-pointer transition-all duration-300 overflow-hidden ${dragOver?"border-[#6DF0C2]/50 bg-[#6DF0C2]/[0.06] scale-[1.015] shadow-[0_8px_32px_rgba(109,240,194,0.15)]":"border-white/10 bg-white/[0.02] hover:bg-white/[0.04] hover:border-white/15 backdrop-blur"}`}>
            ...
            {img ? <img src={img} alt="preview" className="w-full h-[300px] object-cover rounded-2xl"/> : <div className="text-center py-8 relative"><div className="mx-auto w-12 h-12 rounded-2xl bg-white text-black grid place-items-center shadow-lg group-hover:scale-105 transition-transform duration-300"><span className="text-[18px] font-light">↑</span></div><div className="mt-4 text-[15px] font-medium tracking-tight text-white">{t.dropTitle}</div><div className="mt-1.5 text-[13px] leading-5 text-white/40 max-w-[280px] mx-auto">{t.dropSub}</div><div className="mt-2 text-[11px] tracking-wide text-white/25">{t.tip}</div><div className="mt-5 grid grid-cols-2 gap-2.5 max-w-[320px] mx-auto"><button type="button" onClick={(e)=>{e.preventDefault(); e.stopPropagation(); fileRef.current?.click();}} className="py-3 rounded-full bg-white text-black text-[13px] font-semibold cursor-pointer relative z-10 hover:bg-white/90 transition-colors shadow-sm">{t.upload}</button><button type="button" onClick={(e)=>{e.preventDefault(); e.stopPropagation(); cameraRef.current?.click();}} className="py-3 rounded-full bg-white/[0.06] border border-white/10 text-[13px] font-medium cursor-pointer relative z-10 hover:bg-white/[0.08] transition-colors backdrop-blur">{t.camera}</button></div></div>}
          </div>
    ...
    <section className="relative mx-auto max-w-[1100px] px-5 mt-10">
      <div className="rounded-[28px] border border-white/10 bg-white/[.04] backdrop-blur p-8 text-center">
        <div className="w-12 h-12 mx-auto rounded-2xl bg-white text-black grid place-items-center font-black text-sm">KM.</div>
