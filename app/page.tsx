...
export default function Page(){
  ...
  const analyze=async()=>{
    if(!file) return;
    setLoading(true); setRes(null); setRaw(""); setElapsed(0);
    const start=Date.now();
    const timer=setInterval(()=>setElapsed(Math.floor((Date.now()-start)/1000)), 1000); setLiveTrail([]);
    const trailInterval = setInterval(()=>{
      setLiveTrail(prev=>{
        const providers = ["gemini-3.1","ling-3.0-flash-vl","nemotron","aya-vision","qwen2.5-vl"];
  ...
  return (<div className="min-h-screen bg-[#050608] text-white">
    ...
    <section className="relative mx-auto max-w-[1100px] px-5 pt-10 pb-6">
      <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full border text-xs ${status.reachable?"bg-[#6DF0C2]/10 border-[#6DF0C2]/20 text-[#6DF0C2]":"bg-amber-400/10 border-amber-400/20 text-amber-200"}`}>"● Connected — Online"</div>
      ...
      <div className="rounded-[28px] border border-white/10 bg-white/[.04] backdrop-blur p-6 grid lg:grid-cols-[1.1fr_.9fr] gap-6">
        <div>
          <div onClick={()=>fileRef.current?.click()} onDragEnter={(e)=>{e.preventDefault(); setDragOver(true);}} onDragLeave={(e)=>{e.preventDefault(); setDragOver(false);}} onDragOver={(e)=>{e.preventDefault(); e.stopPropagation(); setDragOver(true);}} onDrop={(e)=>{e.preventDefault(); e.stopPropagation(); setDragOver(false); let f:any = e.dataTransfer.files?.[0]; if(!f && e.dataTransfer.items){ for(const it of Array.from(e.dataTransfer.items) as any){ if(it.kind==="file"){ const ff=it.getAsFile(); if(ff && ff.type.startsWith("image/")){ f=ff; break; } } } } if(!f) f = e.dataTransfer.files?.[0] as any; if(f && f.type.startsWith("image/")) onFile(f); else if(f) onFile(f);}} className={`rounded-2xl border-2 border-dashed p-6 cursor-pointer transition ${dragOver?"border-[#6DF0C2] bg-[#6DF0C2]/10 scale-[1.01]":"border-white/15 bg-black/30 hover:bg-white/[.03]"}`}>
            <input ref={fileRef} type="file" accept="image/*" className="hidden" onChange={e=>{const f=e.target.files?.[0]; if(f) onFile(f); e.target.value="";}}/><input ref={cameraRef} type="file" accept="image/*" capture="environment" className="hidden" onChange={e=>{const f=e.target.files?.[0]; if(f) onFile(f); e.target.value="";}}/>
            {img ? <img src={img} alt="preview" className="w-full h-[300px] object-cover rounded-xl"/> : <div className="text-center py-10"><div className="mx-auto w-14 h-14 rounded-2xl bg-white text-black grid place-items-center">⬆</div><div className="mt-3 font-medium">Drop image or click to browse</div><div className="text-sm text-white/50">JPG • PNG • WEBP — <b className="text-white">drop here</b>, click, or <b className="text-white">paste (Ctrl+V anywhere)</b></div><div className="mt-1 text-xs text-white/30">Tip: screenshot → Ctrl+C → Ctrl+V | Mobile: Upload for gallery, Camera for photo</div><div className="mt-3 grid grid-cols-2 gap-2"><button onClick={(e)=>{e.stopPropagation(); fileRef.current?.click();}} className="py-2.5 rounded-xl bg-white text-black text-sm font-bold">⬆ Upload Photo</button><button onClick={(e)=>{e.stopPropagation(); cameraRef.current?.click();}} className="py-2.5 rounded-xl bg-white/[.1] border border-white/10 text-sm font-bold">📷 Camera</button></div></div>}
          </div>
          ...
          <div className="mt-2 text-xs text-center text-white/40">"AI online — detects everything in photo"</div>
        </div>
        <div className="space-y-3">
          <div className="rounded-2xl bg-black border border-white/10 p-4 font-mono text-xs leading-6"><div className="text-white/50">API</div><div>POST /api/analyze</div><div className="text-[#6DF0C2]">→ AI (free)</div><div>→ JSON: whatSeen, isCivic, detections[]</div></div>
